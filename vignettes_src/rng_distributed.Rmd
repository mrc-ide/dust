...
The random number generator in `dust` is well suited to parallel calculations. On a single node we can set up a generator with many streams (either one per core or, typically better, one per logical unit of calculation) and then parallelise the calculation using OpenMP. This is the approach taken by dust objects (`vignette("dust")`) and described for standalone use of the generators (`vignette("rng_package")`).

Some additional care is required to use multiple nodes for the calculation ("distributed parallel computation"), and this vignette explains the problems and solutions. It applies to both use with `dust` objects and for standalone use of the random number generators.
...
```{r}
r2 <- dust::dust_rng$new(seed = 1, n_generators = 4)
matrix(r2$state(), 32)
```
...
We also have coefficients for a "long jump", which is equivalent to 2^192 steps of the generator. The point of the original jump is that individual draws of random numbers will never overlap; the point of the long jumps is that each long jump is so far that we'll never reach the next one by normal jumping (there are 2^64 ~= 10^19 jumps within a single long jump).
...
Next, we need a parallel cluster; we'll use the `parallel` package for this and create a cluster of four nodes.  This might be done via a cluster scheduler or the more sophisticated tools in the `future` package.
...
As before, this gives us four answers (one per node) but each node ran 32 * 1e6 random draws, spread over 8 threads.
...
We could also write a very simple MCMC algorithm:
...
This produces a list of samples from each of the 8 chains, run over 4 nodes, each of which ran using 2 threads.  We can scale any of this parallelism (increasing threads until we hit the total number of particles, increasing nodes until we hit the number of chains) but the results will be deterministic (except for the call to `runif` that we use here for the acceptance test within the MCMC).
...
[^1] It's hard to get a sense of these numbers but `2^128` is about `10^38` draws. If we draw a billion numbers a second (the rate on a reasonable CPU) this would take 3 x 10^21 years to reach the next stream, which is a small fraction of the age of the universe (13 x 10^9 years).
