...
dust_class <- R6::R6Class(
  ...
  public = list(
    ...
    ##'
    ##' @param advance Logical, indicating if the act of reading the RNG
    ##' should cause it to advance the RNG state. If `FALSE` then reading the
    ##' RNG state has no effect on the state, and the state that you retrieve
    ##' should not be used in another dust object as it is perfectly
    ##' correlated with your current dust object (the exception being if
    ##' you want to record your state for reproducibility purposes). If
    ##' `TRUE`, then immediately after reading the state, we do a "long jump"
    ##' on the generator (see [`dust_rng`] for details). By doing this the two
    ##' generators stay uncoupled.
