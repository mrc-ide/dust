...
// TODO: upstream uses we use different constants for the ++ generator
// to the ** or + generator, using ** here as the reference.

// Jump coefficients.
// * 2^64 calls to rng_next
// * 2^64 non-overlapping subsequences
template <>
constexpr std::array<uint64_t, 2> jump_constants<uint64_t, 2>() {
  return std::array<uint64_t, 2>{
    0xdf900294d8f554a5, 0x170865df4b3201fc };
}

// Long-jump coefficients.
// * 2^96 calls to rng_next
// * 2^32 starting points, each with 2^32 subsequences
template <>
constexpr std::array<uint64_t, 2> long_jump_constants<uint64_t, 2>() {
  return std::array<uint64_t, 2>{
    0xd2a98b26625eee7b, 0xdddf9b1090aa7ac1 };
}

template <>
inline HOSTDEVICE uint64_t rng_next(xoroshiro128starstar_state& state) {
...
template <>
inline HOSTDEVICE uint64_t rng_next(xoroshiro128plusplus_state& state) {
  const uint64_t s0 = state[0];
  uint64_t s1 = state[1];
...
template <>
inline HOSTDEVICE uint64_t rng_next(xoroshiro128plus_state& state) {
  const uint64_t s0 = state[0];
  uint64_t s1 = state[1];
