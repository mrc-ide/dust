...
extern "C" void {{name}}_finalise(SEXP ptr) {
  dust::Dust<{{type}}> *obj = dust::util::read_r_pointer<dust::Dust<{{type}}>>(ptr, false);
  if (obj) {
    delete obj;
  }
  if (ptr) {
...
extern "C" SEXP {{name}}_alloc(SEXP sd, SEXP r_n_particles, SEXP r_seed) {
  ...
  dust::Dust<{{type}}> *d = new dust::Dust<{{type}}>(sd, index_y, n_threads, seed, n_particles);
...
extern "C" SEXP {{name}}_run(SEXP ptr, SEXP r_step_end) {
  ...
  dust::Dust<{{type}}> *obj = dust::util::read_r_pointer<dust::Dust<{{type}}>>(ptr, true);
  obj->run(step_end);
