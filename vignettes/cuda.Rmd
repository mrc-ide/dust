...
* we would have to compile support for many possible GPU architechures at once
...
* you will probably want a (much) larger number of particles to take advantage of your device
...
Next, within the dust namespace again, we declare the size of the *shared parameters* for the model. These are parameters that will be the same across all instances of a parmeter set, as opposed to quantities that change between particles.
...
In the CPU version of the model we have a nice smart pointer to a struct (`dust::shared_ptr<sirs>`) from which we can access parameters by name (e.g., `shared->alpha`). No such nicities in CUDA where we need access to a single contiguous block of memory.  The `dust::shared_copy` is a small utility to make the bookkeeping here a bit easier, but this could have been written out as:
...
* the presence of the four auxillary data elements (`internal_int`, `internal_real`, `shared_int` and `shared_real`)
* the data types that vary across particles are a special `dust::interleaved<>` type, which prevents uncoalesced reads
* All accesses into the `shared_int` and `shared_real` elements are now by position in the array, rather than the previous pointer/name based acces
