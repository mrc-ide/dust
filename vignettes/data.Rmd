---
title: "dust: Comparing models and data"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{dust: Comparing models and data}
...
This vignette outlines the steps in implenenting the comparison directly as part of the model.  This is not required for basic use of dust models, and we would typically recommend this only after your model has stabilised and you are looking to extract potential additional speedups or accellerate the model on a GPU.
...
As in the [`mcstate` vignette](https://mrc-ide.github.io/mcstate/articles/kalman.html) we some way of comparing model output to data.  The function used there is:
...
* describing the data (here it will be a single numeric value, but the data could be arbitraricly complicated)
* implementing a method to do the comparison
* describe how to marshall the data into this structure
...
This structure can contain whatever you want, including things like a `std::vector` of values. In our use we've typically only had `real_t` and `int` though, even for complex models.
...
* `dnorm` is replaced with the dust function `dust::dnorm` (do not use R API functions here, as this will be evaluated in a multi-threaded context)
...
Finally, the data mashalling is done by the `dust_data` template, within the `dust` namespace
...
typename T::real_t compare(const typename T::real_t * state,
  ...
  return dust::dnorm(data.observed, odin(gamma) * odin(value), odin(tau), true);
}
```

and the correct code would be generated.
...
```{r}
mod$reset(list(alpha = 0.91, sigma = 1), 0)
