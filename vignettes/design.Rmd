...
```c++
  ...
  void update(size_t step, const real_t * state,
    ...
    for (size_t i = 0; i < shared->n; ++i) {
      internal.samples[i] = dust::distr::unif_norm(rng_state)
    }
...
* Reset particles and the components of their state (`$update_state`) which includes parameters, state vectors and time.
...
* Reordering the particles (`$reorder`) - shuffles particle state among particles within a parameter set. This is useful for implementing a resampling algorithms and updates only the state (as for `$set_state`, leaving RNG state and internal state untouched)
