...
In order to run the simulation beginning-to-end, we use `dust::dust_iterate()`; this function is purely a convenience function and the intention is that you would write your own (see [`mcstate`](https://mrc-ide.github.io/mcstate) for example).

```{r sir_run_collect}
model <- sir$new(list(), 0, 200)
steps <- seq(0, 600, by = 5)
state <- dust::dust_iterate(model, steps, 2L)
```
