...
There are two main parts here; the class (`walk`) and an interface function (`dust_pars`).
...
* the type `shared_t` is whatever data your model needs that is shared across a number of particles, and unmodified during the model update (most likely parameters), and an object of this type will be passed to your constructor as the `.shared` field to `pars` (see `dust_pars` below, and see `vignette("multi")` for more on parameters)
...
We use as many random number generators as their are particles, so if you run fewer particles or more, increase the threads or decrease, the results will be the same (see `vignette("design")` for more on this).
...
There is a bit different here to the version that is shown before for the walk model, but the core parts are only slightly different:
