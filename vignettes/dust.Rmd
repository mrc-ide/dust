...
* the type `shared_t` is whatever data your model needs that is *shared across all particles*, and unmodified during the model update (most likely parameters), and an object of this type will be passed to your constructor as the `.shared` field to `pars` (see `dust_pars` below)
* the type `internal_t` is whatever internal data your model needs (most likely scratch space used for intermediate calculations), and an object of this type will be passed to your constructor as the `.internal` field to `pars` (see `dust_pars` below)
