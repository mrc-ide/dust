...
The class is what does most of the work. It is important that this does not use anything in the R api as it may be called in parallel. Therefore use only C++ standard library types.  It has to provide every public type or method as the example above. First, the four types:
...
* the type `rng_state_type` is one of the valid RNG types. Typically `typedef dust::random::generator<real_type> rng_state_type;` will select a reasonable choice, but you could also force a specific generator such as `dust::random::xoshiro256starstar_state` here (see `vignette("rng")`)
...
At this point our particles have been run for 100 steps with standard deviation 1 at each step so they [will be distributed following Normal(0, 10)](https://en.wikipedia.org/wiki/Random_walk#Gaussian_random_walk).  This is easier to see if we simulate a lot of particles, here 20 thousand:
...
A particle state is determinied by three mutable things; `pars`, `state` and `step`; these can all be updated for a model after it has been created.  We have found setting one or more of these at a time important;

* Resetting the model with a new set of parameters (`pars`), initial conditions (`state`) and times (`step`)
...
* Setting `step` along with `state` when initialising the model from a previously saved state
...
One particulary common case is to "reset" the model in order to run it again. you should not simply recreate the model from its constructor as that will re-seed a new set of random state -- it would be preferable continue with the genreator state in your old model.

Suppose we run our model with sd of 1 for 100 steps
...
```{r}
model$update_state(pars = list(sd = 2), step = 0)
y2 <- model$run(100)
```
