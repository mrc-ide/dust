...
* the type `init_t` is whatever internal data your model needs (most likely its parameters), and an object of this type will be passed to your constructor (see `dust_pars` below)
* the constructor must take only one argument, being `const init_t&`. You can do what you want in the constructor - here we just copy the pars into the object (we use the private member `pars_` here but this can be anything you want)
...
The function below the class is a template specialisation of a dust function `dust_pars`; it is used to create your `init_t` object from an R list (`cpp11::list`). You can include `cpp11.hpp` (or more narrowly scoped cpp11 headers) at this point and can use any cpp11 function that you need. Avoid direct use of the R's C API as this is not always safe to call from C++. In particular, use `cpp11::stop()` rather than `Rf_error()`.
...
The other difference is a new template specialisation of `dust_info` - this is optional but can be used to report back to R information about the model based on the input pars. Here it returns information about variable order and core parameters.
