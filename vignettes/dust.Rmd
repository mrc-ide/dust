...
* the type `real_type` has to exist and indicate what sort of numeric type you use for "real numbers". Typically this will be `double`,but if you need a different type (longer or shorter) you can use it here.
* the type `data_type` which for now should match exactly the construction `typedef dust::no_data data_type` until we stabilise and document this interface
...
* the type `rng_state_type` is one of the valid RNG types. Typically `typedef dust::random::generator<real_type> rng_state_type;` will select a reasonable choice (see `vignette("rng")`)
...
* there is a `compare_data` method and a nontrivial `data_type` definition, but ignore these for now
