...
Ordinarily this is used from C++; the model as discussed in `vignette("dust")` uses `dust::RNG<real_t, int_t>` to interact with the generator. However, an R interface is provided for debugging and testing purposes.
...
```cc
int_t n = rng.rpois(4.5)
```

**Binomial distribution** with mean `size` and `prob`

```{r}
rng$rbinom(10, 10, 0.3)
```

```cc
int_t n = rng.rbinom(10, 0.3)
```
...
**Standard normal distribution** (mean 0 and sd 1) - faster than using `$rnorm(n, 0, 1)`:
...
Practically, performance will vary based on the parameters of the distributions and the underlying algorithms, and the principle performance gain we hope to get here is driven by the ability to parallelise safely.
