...
Ordinarily this is used from C++; the model as discussed in `vignette("dust")` uses `rng_state_t` and a function from `dust::random` to interact with the generator. However, an R interface is provided for debugging and testing purposes.
...
```cc
int n = rng.binomial<real_t>(10, 0.3);
```

There are also helpers for some common special case distributions:

**Standard uniform distribution** (between 0 and 1) - faster than using `$uniform(n, 0, 1)`:
